...
In the current no-API architecture, this script does not call Claude.
Articles are merely surfaced to a human via Telegram; the human then
opens Claude.ai and applies the prompts in scripts/analyzer/prompts/
manually. When ANTHROPIC_API_KEY is added to repo secrets and an
autonomous analyzer is built, this orchestrator will gain an additional
step between scraper and notifier.
