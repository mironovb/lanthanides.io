...
const DESCRIPTION =
  `All ${RECORD_COUNT} sourced price observations for 31 rare-earth and strategic-metal elements on a single log-scale USD per kg axis. Retail and bulk shown separately, with observed ranges, quartile bands, and medians. Open data, CC BY 4.0.`;
  ...
  return (
    <Container as="main" className="py-10">
      ...
      <PageHeader
        ...
        eyebrow="Tools"
        title="Price Map"
        lead={`Every sourced price in the ledger, drawn on one axis. One row per element, one mark per observation. Retail and bulk are different markets, shown as separate tracks and never merged. The axis is logarithmic because observed prices span ${spanOrders} orders of magnitude, from ${fmtUsdPrice(minPrice)} to ${fmtUsdPrice(maxPrice)} per kg.`}
        actions={
      ...
      <div className="mt-10">
        <Callout tone="note" title="Observed spread is not a fair-price estimate">
          ...
          with plain, unweighted statistics. The{' '}
          <Link
            href="/tools/price-gauge/"
            className="text-accent hover:text-accent-strong"
          >
