...
**lanthanides.io — Strategic Materials Ledger** is an **open, source-cited
price reference** for rare-earth and strategic materials (elements, prices,
provenance, regulatory intelligence), built as a **Next.js** app migrated from
the original static **Jekyll** site. **The main goal: a reference price that
can be assembled from community contributions** — source-cited observations,
...
## Data strategy (the core decision)

**One store.** All reference + provenance data lives in versioned files —
`_data/`, `_elements/`, `_articles/`. Read at build time through the typed data
layer in **`lib/data/`** (contracts in `lib/types.ts`, mirroring ARCHITECTURE
...
scripts/      UNCHANGED — Python pipeline (scheduled PR updates for _data/)
public/       assets/images (favicons, og, manifest), assets/data (open-data exports, build-generated)
docs/         AUDIT.md, MIGRATION.md, ARCHITECTURE.md, DEPLOYMENT.md, …
              (legacy/ — the quarantined Jekyll tree — was removed in Prompt 25;
               prisma/ — the DB layer — was removed in the 2026-07-02 refocus)
```
...
**Historical record.** The Jekyll→Next migration completed at Prompt 25; a
redesign pass followed; the **2026-07-02 refocus** then removed the commercial
DB layer these entries describe (sell/offers/alerts/discussion, Prisma). Read
the entries as what was built at the time, not as the current app. Each prompt
must leave `npm run build` green.
