...
// Scroll-progress gain. The kit default (4) suits very long pages; the about
// page is short, so 1.5 lets the star form over roughly the first third of
// the scroll instead of the first hundred pixels.
const SCROLL_GAIN = 1.5;

// Brand palette (matches the site accent and ink tokens in tailwind.config.ts:
// accent #1a5c6b, neutral #6c757d, fg #1a1a1a). The kit shipped orange; the
// electrons/protons carry the one teal accent, everything else stays ink/gray.
const COLOR_ELECTRON = "#1a5c6b";
const COLOR_ELECTRON_EMISSIVE = "#0d2e36";
const COLOR_PROTON = "#1a5c6b";
const COLOR_PROTON_EMISSIVE = "#071a1f";
const COLOR_NEUTRON = "#6c757d";
const COLOR_RING = "#1a1a1a";
...
export default function LanthanumBohrScroll({
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      scrollRef.current = clamp01(raw * SCROLL_GAIN);
    };
  ...
  return (
    ...
    >
      ...
      >
        {/* Fill lights re-tinted from the kit's warm orange to cool teal so the
            accent-colored electrons stay on-hue. */}
        <ambientLight intensity={0.85} />
        <directionalLight position={[6, 7, 6]} intensity={1.1} />
        <pointLight position={[-5, -2, 3]} intensity={0.55} color="#d7ecf1" />
        <pointLight position={[0, 0, 0]} intensity={0.5} distance={5} color="#3f93a8" />
